...
<div>{author}</div>
    ...
    }
    {
        ...
        <><input onChange={e=>{setDropdown(users.filter(x=>x.username===e.target.value))}}/>
        <div className="dropdown">{dropdown.map((e)=><div>{e.username}</div>)}{ console.log(dropdown)}</div>
        </>
    }
